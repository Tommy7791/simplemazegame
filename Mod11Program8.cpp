...
SDL_Texture* textureZombieNorthSheet = NULL;     //sheet of east zombie texture
SDL_Texture* textureZombieSouthSheet = NULL;     //sheet of west zombie texture
SDL_Texture* textureZombieSheetNS = NULL;         //generic sheet of zombie texture
...
SDL_Rect rectFarmerPlacementNS;   //Where to place the e/w farmer on the screen
SDL_Rect rectFarmerSheetNS;       //Sheet containing e/w farmer for currect direction
SDL_Rect rectFarmerNS;            //Rectangle containing e/w farmer placed on screen
...
SDL_Rect rectZombieNorthSheet;   //Rectangle for sheet of east walking farmer
SDL_Rect rectZombieNorth;        //One east walking farmer rectangle
SDL_Rect rectZombieSouthSheet;   //Rectangle for sheet of west walking farmer
SDL_Rect rectZombieSouth;        //One west walking farmer rectangle
...
SDL_Rect rectZombiePlacementNS;   //Where to place the e/w zombie on the screen
SDL_Rect rectZombieSheetNS;       //Sheet containing e/w zombie for currect direction
SDL_Rect rectZombieNS;            //Rectangle containing e/w zombie placed on screen

//rect explosion
SDL_Rect rectExplosionSheet;
...
int yPiggyPos = 320;            //y position of piggy on screen
...
int xFarmerEWpos = 160;          //x position of e/w farmer on screen
int yFarmerEWpos = 160;         //y position of e/w farmer on screen
bool farmerDestroyed = false;

int SpriteFrameFarmerNS = 0;
int FrameCounterFarmerNS = 0;
int xFarmerNSpos = 330;          //x position of e/w farmer on screen
int yFarmerNSpos = 180;         //y position of e/w farmer on screen
bool farmerDestroyed2 = false;
//Zombie variables
...
int SpriteFrameZombieNS = 0;
int FrameCounterZombieNS = 0;
int xZombieNSpos = 330;          //x position of e/w zombie on screen
int yZombieNSpos = 450;         //y position of e/w zombie on screen
bool zombieDestroyed2 = false;
...
void destroyFarmer();
void destroyFarmerNS();
void destroyPiggy();
void destroyZombie();
void destroyZombieNS();
...
void destroyFarmerNS()
{
    ...
    SDL_DestroyTexture(textureFarmerSheetNS);
}
...
void destroyZombieNS()
{
    SDL_DestroyTexture(textureZombieSouthSheet);
    SDL_DestroyTexture(textureZombieNorthSheet);
    SDL_DestroyTexture(textureZombieSheetNS);
}
...
void CreateGameImages()
{
    ...
    "Let's help the poor piggy out of the maze...!!!", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
    ...
    //create the surface - load the west sheet
    tempSurface = loadImage("graphics/WalkNorth/northZombie.bmp");
    //Set magenta as transparency color
    tempSurface = setTransparentColor(tempSurface, 255, 255, 255);
    //create the texture for the whole sheet
    textureZombieNorthSheet = loadTexture(tempSurface);
    //Create the rectangle to hold one zombie walking west
    //SDL_QueryTexture() method gets the width and height of the sheet of zombie
    SDL_QueryTexture(textureZombieNorthSheet, NULL, NULL, &rectZombieNorthSheet.w, &rectZombieNorthSheet.h);
    //get the width of one frame (one zombie) by dividing the width of the sheet by the number of frames
    zombieWidth = rectZombieNorthSheet.w/MAX_ZOMBIE_FRAME;
    //get the height of one frame
    zombieHeight = rectZombieNorthSheet.h;  //height on one zombie is same as height of sheet
    //Initialize the rectangle zombie west
    FillRect(rectZombieNorth, 0, 0, zombieWidth, zombieHeight);

    //create the surface - load the west sheet
    tempSurface = loadImage("graphics/WalkSouth/southZombie.bmp");
    //Set magenta as transparency color
    tempSurface = setTransparentColor(tempSurface, 255, 255, 255);
    //create the texture for the whole sheet
    textureZombieSouthSheet = loadTexture(tempSurface);
    //Create the rectangle to hold one zombie walking west
    //SDL_QueryTexture() method gets the width and height of the sheet of zombie
    SDL_QueryTexture(textureZombieSouthSheet, NULL, NULL, &rectZombieSouthSheet.w, &rectZombieSouthSheet.h);
    //get the width of one frame (one zombie) by dividing the width of the sheet by the number of frames
    zombieWidth = rectZombieSouthSheet.w/MAX_ZOMBIE_FRAME;
    //get the height of one frame
    zombieHeight = rectZombieSouthSheet.h;  //height on one zombie is same as height of sheet
    //Initialize the rectangle zombie west
    FillRect(rectZombieSouth, 0, 0, zombieWidth, zombieHeight);
    ...
    //get the height of one frame
    explosionHeight = rectExplosionSheet.h;  //height on one zombie is same as height of sheet
    //Initialize the rectangle zombie west
...
void UpdatePiggy()
{
    ...
    playerEnemyCollide(rectPiggyPlacement, rectFarmerPlacementNS);
    //Now check to see if the piggy has collided with an enemy
    playerEnemyCollide(rectPiggyPlacement, rectZombiePlacementEW);
    playerEnemyCollide(rectPiggyPlacement, rectZombiePlacementNS);
}
...
void updateNorthSouthFarmer()
{
    bool collision;
    //Increment time to delay each frame
    FrameCounterFarmerNS++;
    //If delay time is greater than max amount
    if(FrameCounterFarmerNS > FRAME_DELAY)
    {
        FrameCounterFarmerNS = 0;   //start new delay time
        SpriteFrameFarmerNS++;      //advance to next frame
    }
    if(SpriteFrameFarmerNS > MAX_FARMER_FRAME - 1) //if last frame is reached
        SpriteFrameFarmerNS = 0;                   //start at frame zero again

    //Note:  west = 0, east = 1, south = 2, north = 3
    // west = 0
    if (farmerDirectionNS == 3)                       //if we are going east (right)
    {
        //Advance to next frame on sheet
        rectFarmerNorth.x = SpriteFrameFarmerNS * rectFarmerNorth.w;
        yFarmerNSpos = yFarmerNSpos - FARMER_SPEED;              //increment x
        //create the rectangle on screen for farmer placement
        FillRect(rectFarmerNS, rectFarmerNorth.x, rectFarmerNorth.y, rectFarmerNorth.w, rectFarmerNorth.h);
        textureFarmerSheetNS = textureFarmerNorthSheet;
    }// end walking east

    if (farmerDirectionNS == 2)                       //walking to the west (left)
    {
        //Advance to next frame on sheet
        rectFarmerSouth.x = SpriteFrameFarmerNS * rectFarmerSouth.w;
        yFarmerNSpos = yFarmerNSpos + FARMER_SPEED;              //decrement x
        //create the rectangle on screen for farmer placement
        FillRect(rectFarmerNS, rectFarmerSouth.x, rectFarmerSouth.y, rectFarmerSouth.w, rectFarmerSouth.h);
        textureFarmerSheetNS = textureFarmerSouthSheet;
    }// end walking West

    //Now check to see if the farmer has collided with a solid tile
    collision = EnemyTileCollision(rectFarmerPlacementNS, farmerDirectionNS);

    if (collision == true)
    {
        FrameCounterFarmerNS = 0;       //start new position
    }//end if

    //Initialize the placement rectangle to the position on the screen the farmer will be drawn.
    FillRect(rectFarmerPlacementNS, xFarmerNSpos, yFarmerNSpos, farmerWidth, farmerHeight);
}
...
void updateNorthSouthZombie()
{
    bool collision;
    //Increment time to delay each frame
    FrameCounterZombieNS++;
    //If delay time is greater than max amount
    if(FrameCounterZombieNS > FRAME_DELAY)
    {
        FrameCounterZombieNS = 0;   //start new delay time
        SpriteFrameZombieNS++;      //advance to next frame
    }
    if(SpriteFrameZombieNS > MAX_FARMER_FRAME - 1) //if last frame is reached
        SpriteFrameZombieNS = 0;                   //start at frame zero again

    //Note:  west = 0, east = 1, south = 2, north = 3
    // west = 0
    if (zombieDirectionNS == 2)                       //if we are going east (right)
    {
        //Advance to next frame on sheet
        rectZombieSouth.x = SpriteFrameZombieNS * rectZombieSouth.w;
        yZombieNSpos = yZombieNSpos + ZOMBIE_SPEED;              //increment x
        //create the rectangle on screen for zombie placement
        FillRect(rectZombieNS, rectZombieSouth.x, rectZombieSouth.y, rectZombieSouth.w, rectZombieSouth.h);
        textureZombieSheetNS = textureZombieSouthSheet;
    }// end walking east

    if (zombieDirectionNS == 3)                       //walking to the west (left)
    {
        //Advance to next frame on sheet
        rectZombieNorth.x = SpriteFrameZombieNS * rectZombieNorth.w;
        yZombieNSpos = yZombieNSpos - ZOMBIE_SPEED;              //decrement x
        //create the rectangle on screen for zombie placement
        FillRect(rectZombieNS, rectZombieNorth.x, rectZombieNorth.y, rectZombieNorth.w, rectZombieNorth.h);
        textureZombieSheetNS = textureZombieNorthSheet;
    }// end walking West

    //Now check to see if the zombie has collided with a solid tile
    collision = EnemyTileCollision(rectZombiePlacementNS, zombieDirectionNS);

    if (collision == true)
    {
        FrameCounterZombieNS = 0;       //start new position
    }//end if

    //Initialize the placement rectangle to the position on the screen the zombie will be drawn.
    FillRect(rectZombiePlacementNS, xZombieNSpos, yZombieNSpos, zombieWidth, zombieHeight);
}
...
void updateExplosion()
{
    ...
    FillRect(rectExplosionPlacement, rectExplosion.x, rectExplosion.y, rectExplosion.w, rectExplosion.h);
...
void DrawGame()
{
    ...
    SDL_RenderCopy(renderer, textureFarmerSheetNS, &rectFarmerNS, &rectFarmerPlacementNS);

    SDL_RenderCopy(renderer, textureZombieSheetEW, &rectZombieEW, &rectZombiePlacementEW);
    SDL_RenderCopy(renderer, textureZombieSheetNS, &rectZombieNS, &rectZombiePlacementNS);
...
void playerEnemyCollide(SDL_Rect playerRect, SDL_Rect enemyRect)
{
    ...
    const SDL_Rect& CONST_ZOMBIE_RECT2 = rectZombiePlacementNS;
    ...
    const SDL_Rect& CONST_FARMER_RECT2 = rectFarmerPlacementNS;
   ...
   if (CheckCollision(playerRect, enemyRect))
    {
        ...
        if (SDL_RectEquals(&ENEMY_RECT, &CONST_ZOMBIE_RECT2))
        {
            zombieDestroyed2 = true;
            destroyZombieNS();
        }
        ...
        if (SDL_RectEquals(&ENEMY_RECT, &CONST_FARMER_RECT2))
        {
            farmerDestroyed2 = true;
            destroyFarmerNS();
        }
    }
}
